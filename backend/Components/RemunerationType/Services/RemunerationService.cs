namespace backend.RemunerationType.Services;
...
using backend.RemunerationType.Repositories.Interfaces;
...
public class RemunerationTypeService : IRemunerationTypeService
{
    ...
    public async Task<RemunerationType> CreateRemunerationTypeAsync(string type)
    {
        ...
        if (existingTypes.Any(r => r.GetType().Equals(type, StringComparison.OrdinalIgnoreCase)))
